...
contract MetaRise is ERC20, Ownable {
	...
	// max transaction is 0.5% of initialSupply
	uint256 public maxTxAmount = initialSupply * 500 / 100000;
	...
	// Base taxes
	CustomTaxPeriod private _default = CustomTaxPeriod('default',0,0,1,1,2,3,1,3,5,5);
	CustomTaxPeriod private _base = CustomTaxPeriod('base',0,0,1,1,2,3,1,3,5,5);
	...
	event FeesApplied(uint8 liquidityFee, uint8 marketingFee, uint8 buybackFee, uint8 holdersFee, uint8 totalFee);
	...
	constructor() ERC20(_name, _symbol) {
		...
		dividendTracker = new MetRiseDividendTracker();
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "MetRise: Automated market maker pair is already set to that value");
	...
	function excludeFromFees(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromFee[account] != excluded, "MetRise: Account is already the value of 'excluded'");
	...
	function excludeFromMaxTransactionLimit(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromMaxTransactionLimit[account] != excluded, "MetRise: Account is already the value of 'excluded'");
		_isExcludedFromMaxTransactionLimit[account] = excluded;
		emit ExcludeFromMaxTransactionChange(account, excluded);
	}
	function excludeFromMaxWalletLimit(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromMaxWalletLimit[account] != excluded, "MetRise: Account is already the value of 'excluded'");
		_isExcludedFromMaxWalletLimit[account] = excluded;
		emit ExcludeFromMaxWalletChange(account, excluded);
	}
	function blockAccount(address account) external onlyOwner {
		require(!_isBlocked[account], "MetRise: Account is already blocked");
		_isBlocked[account] = true;
		emit BlockedAccountChange(account, true);
	}
	function unblockAccount(address account) external onlyOwner {
		require(_isBlocked[account], "MetRise: Account is not blocked");
	...
	function setWallets(address newLiquidityWallet, address newmarketingWallet, address newBuyBackWallet) external onlyOwner {
		if(liquidityWallet != newLiquidityWallet) {
			require(newLiquidityWallet != address(0), "MetRise: The liquidityWallet cannot be 0");
			emit WalletChange('liquidityWallet', newLiquidityWallet, liquidityWallet);
			liquidityWallet = newLiquidityWallet;
		}
		if(marketingWallet != newmarketingWallet) {
			require(newmarketingWallet != address(0), "MetRise: The marketingWallet cannot be 0");
			emit WalletChange('marketingWallet', newmarketingWallet, marketingWallet);
			marketingWallet = newmarketingWallet;
		}
		if(buyBackWallet != newBuyBackWallet) {
			require(newBuyBackWallet != address(0), "MetRise: The buyBackWallet cannot be 0");
	...
	function setFeeOnSelectedWalletTransfers(address account, bool value) external onlyOwner {
		require(_feeOnSelectedWalletTransfers[account] != value, "MetRise: The selected wallet is already set to the value ");
		_feeOnSelectedWalletTransfers[account] = value;
		emit FeeOnSelectedWalletTransfersChange(account, value);
	}
	function setUniswapRouter(address newAddress) external onlyOwner {
		require(newAddress != address(uniswapV2Router), "MetRise: The router already has that address");
		emit UniswapV2RouterChange(newAddress, address(uniswapV2Router));
		uniswapV2Router = IRouter(newAddress);
	}
	function setMaxWalletAmount(uint256 newValue) external onlyOwner {
		require(newValue != maxWalletAmount, "MetRise: Cannot update maxWalletAmount to same value");
		emit MaxWalletAmountChange(newValue, maxWalletAmount);
		maxWalletAmount = newValue;
	}
	function setMaxTransactionAmount(uint256 newValue) external onlyOwner {
		require(newValue != maxTxAmount, "MetRise: Cannot update maxTxAmount to same value");
		emit MaxTransactionAmountChange(newValue, maxTxAmount);
		maxTxAmount = newValue;
	}
	function setMinimumTokensBeforeSwap(uint256 newValue) external onlyOwner {
		require(newValue != minimumTokensBeforeSwap, "MetRise: Cannot update minimumTokensBeforeSwap to same value");
	...
	function _transfer(
		...
		uint256 amount
		    ...
		    if(!_isAllowedToTradeWhenDisabled[from] && !_isAllowedToTradeWhenDisabled[to]) {
				require(isTradingEnabled, "MetRise: Trading is currently disabled.");
				require(!_isBlocked[to], "MetRise: Account is blocked");
				require(!_isBlocked[from], "MetRise: Account is blocked");
				if (!_isExcludedFromMaxWalletLimit[to]) {
					require((balanceOf(to) + amount) <= maxWalletAmount, "MetRise: Expected wallet amount exceeds the maxWalletAmount.");
				}
				if (!_isExcludedFromMaxTransactionLimit[to] && !_isExcludedFromMaxTransactionLimit[from]) {
					require(amount <= maxTxAmount, "MetRise: Buy amount exceeds the maxTxAmount.");
				}
			}
	...
	function _adjustTaxes(bool isBuyFromLp, bool isSelltoLp, address from, address to) private {
		...
		emit FeesApplied(_liquidityFee, _marketingFee, _buyBackFee, _holdersFee, _totalFee);
	}
	...
	function _swapAndLiquify() private {
		...
		uint256 amountETHHolders = ETHBalanceAfterSwap - (amountETHLiquidity + amountETHMarketing + amountETHBuyBack);
...
contract MetRiseDividendTracker is DividendPayingToken {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() DividendPayingToken("MetRise_Dividend_Tracker", "MetRise_Dividend_Tracker") {
		claimWait = 3600;
		minimumTokenBalanceForDividends = 0 * (10**18);
	}
	function _transfer(address, address, uint256) internal pure override {
		require(false, "MetRise_Dividend_Tracker: No transfers allowed");
	}
	function excludeFromDividends(address account) external onlyOwner {
	...
	function setTokenBalanceForDividends(uint256 newValue) external onlyOwner {
		require(minimumTokenBalanceForDividends != newValue, "MetRise_Dividend_Tracker: minimumTokenBalanceForDividends already the value of 'newValue'.");
